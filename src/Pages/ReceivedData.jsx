...
const ReceivedData = () => {
  ...
  if (isLoading) return <div className="loading-message">Loading Received Data...</div>;
  if (error) return <div className="error-message">{error}</div>;

  return (
    <div className="received-data">
      <h1 className="page-title">Received Data</h1>
      ...
      {receivedData.length > 0 ? receivedData.map((item, index) => (
        <div key={item._id} className="company-section">
          ...
          <div className={`company-content ${expandedCompanies[item._id] ? 'expanded' : ''}`}>
            ...
            <button className="edit-team-btn">Create Project</button>
        ...
        <p className="no-data-message">No received data found.</p>
      )}
    </div>
  );
};
